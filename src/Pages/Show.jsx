...
export default function Show() {
  ...
  const predictPrice = async() => {
    const res = await axios.get(`https://samarth.pythonanywhere.com/?cur=${store.data.symbol}`);
    // console.log(typeof(res.data));
    setData(res.data.next_day_prediction);
    console.log(res.data);
  }

  useEffect(() => {
    store.fetchData(params.id);
    // fetch("http://127.0.0.1:7777/").then(
    //   res => res.json()
    // ).then(
    //   data => {
    //     setData(data)
  ...
  return (
    <div>
      ...
      <div className='show-details-row'>
        <button type = 'submit' onClick={predictPrice}>Predict next day price</button>
        <span>{data}</span>
      </div>
      </div>
